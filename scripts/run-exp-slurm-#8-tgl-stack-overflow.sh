...
#SBATCH --job-name=stack-overflow-tgl-8
#SBATCH --output=stack-overflow-tgl-8.out
#SBATCH --error=stack-overflow-tgl-8.err
### GPU options ###
...
for model in TGAT APAN TGN JODIE; do
    ...
    srun python -m torch.distributed.launch --nproc_per_node=9 train_dist.py  --data stack-overflow --config "/u/wanyu/github/tgl/examples/exp/dist/$model.yml" --num_gpus $num_gpus;
